...
public class CertificateDownloader {
	 ...
	 * @throws SQLException
	 */
	private static void insertNewCertificate(Connection sqlite,
	 ...
	 * @throws SQLException
	 */
	private static void prepareScanStatement(Connection sqlite,
	                                         CertificateResponse response,
	                                         String domain)
			throws CertificateEncodingException, NoSuchAlgorithmException, SQLException {
		final X509Certificate certificate = response.getCertificate();
	...
	public static void main(String[] args) {
		...
		options.addOption(Option.builder("output")
				.hasArg()
				.required()
				.desc("output file location (SQLite database). New file will be created at this location, unless -continue is used.")
				.argName("file")
				.build());
		options.addOption(Option.builder("input")
				.hasArg()
				.desc("input file location (plaintext, one domain per line). If none specified, STDIN will be used.")
				.argName("file")
				.build());
		...
		options.addOption(Option.builder("continue")
				.hasArg()
				.optionalArg(true)
				.desc("don't create new output file. If a domain is specified, don't scan any domain till <domain> is reached in input.")
				.argName("domain")
				.build()
		);
		...
		if (cmd.hasOption("threads"))
		{
			...
			if (maxWorkers < 1)
				throw new IllegalArgumentException("The number of threads cannot be lower than 1");
		}
		else
		{
			maxWorkers = 50;
		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				...
				executorService.submit((Runnable) () -> {
					...
					try {
						prepareScanStatement(sqlite, response, domain);
					} catch (NoSuchAlgorithmException ignored) {
						System.err.println("Unable to calculate the thumbprint of the certificate (SHA-1)");
						semaphore.release();
						return;
